...
namespace UnityGLTF
{
    ...
    public class GLTFSceneImporter
    {
        ...
        protected readonly GLTF.Schema.GLTFMaterial DefaultMaterial = new GLTF.Schema.GLTFMaterial();
        ...
        private IEnumerator ConstructMeshAttributes(GLTF.Schema.GLTFMesh mesh, MeshId meshId)
        {
        ...
        protected IEnumerator ConstructImageBuffer(GLTF.Schema.GLTFTexture texture, int textureIndex)
        {
            int sourceId = GetTextureSourceId(texture);
            if (_assetCache.ImageStreamCache[sourceId] == null)
            {
                GLTF.Schema.GLTFImage image = _gltfRoot.Images[sourceId];
        ...
        protected IEnumerator _LoadScene(int sceneIndex = -1, bool isMultithreaded = false)
        {
            ...
            if (_lastLoadedScene == null)
            {
                if (_gltfRoot.Buffers != null)
                {
                    ...
                    {
                        GLTF.Schema.GLTFBuffer buffer = _gltfRoot.Buffers[i];
                ...
                if (_gltfRoot.Textures != null)
                {
                    for (int i = 0; i < _gltfRoot.Textures.Count; ++i)
                    {
                        ...
                        {
                            GLTF.Schema.GLTFTexture texture = _gltfRoot.Textures[i];
        ...
        protected IEnumerator ConstructBuffer(GLTF.Schema.GLTFBuffer buffer, int bufferIndex)
        {
            if (buffer.Uri == null)
            {
        ...
        protected byte[] GetImageDataBuffer(GLTF.Schema.GLTFImage image, int imageCacheIndex)
        {
        ...
        protected IEnumerator ConstructImage(GLTF.Schema.GLTFImage image, int imageCacheIndex, FilterMode filterMode, TextureWrapMode wrapMode, bool markGpuOnly = true)
        {
        ...
        protected virtual IEnumerator ConstructAttributesForMeshes()
        {
            ...
            {
                GLTF.Schema.GLTFMesh mesh = _gltfRoot.Meshes[i];
        ...
        protected virtual IEnumerator ConstructMeshAttributes(MeshPrimitive primitive, int meshID, int primitiveIndex)
        {
            if (_assetCache.MeshCache[meshID][primitiveIndex].MeshAttributes.Count == 0)
            {
                ...
                foreach (var attributePair in primitive.Attributes)
                {
                    ...
                    GLTF.Schema.GLTFBuffer buffer = bufferIdPair.Value;
        ...
        protected virtual void BuildAnimationSamplers(GLTF.Schema.GLTFAnimation animation, int animationId)
        {
        ...
        protected virtual IEnumerator ConstructMesh(GLTF.Schema.GLTFMesh mesh, Transform parent, int meshId, Skin skin)
        {
        ...
        protected virtual IEnumerator ConstructMeshPrimitive(MeshPrimitive primitive, int meshID, int primitiveIndex, int materialIndex)
        {
            ...
            GLTF.Schema.GLTFMaterial materialToLoad = shouldUseDefaultMaterial ? DefaultMaterial : primitive.Material.Value;
        ...
        protected virtual IEnumerator ConstructMaterialImageBuffers(GLTF.Schema.GLTFMaterial def)
        {
        ...
        protected virtual IEnumerator ConstructMaterialTextures(GLTF.Schema.GLTFMaterial def)
        {
        ...
        protected IEnumerator ConstructUnityMesh(MeshConstructionData meshConstructionData, int meshId, int primitiveIndex)
        {
			...
			UnityEngine.Mesh mesh = new UnityEngine.Mesh
			{
                ...
                triangles = primitive.Indices != null
                    ...
                    : MeshPrimitive.GenerateTriangles(vertexCount), // Because send an int[]
                ...
                boneWeights = meshAttributes.ContainsKey(SemanticProperties.Weight[0]) && meshAttributes.ContainsKey(SemanticProperties.Joint[0])
                    ? CreateBoneWeightArray(meshAttributes[SemanticProperties.Joint[0]].AccessorContent.AsVec4s.ToUnityVector4Raw(),
                        meshAttributes[SemanticProperties.Weight[0]].AccessorContent.AsVec4s.ToUnityVector4Raw(), (int)vertexCount)
                    : null
            };
        ...
        protected virtual void ConstructMaterial(GLTF.Schema.GLTFMaterial def, int materialIndex)
        {
            IUniformMap mapper;
        ...
        protected virtual int GetTextureSourceId(GLTF.Schema.GLTFTexture texture)
        {
        ...
        public virtual IEnumerator LoadTexture(GLTF.Schema.GLTFTexture texture, int textureIndex, bool markGpuOnly = true)
        {
            try
            {
                lock (this)
        ...
        protected virtual IEnumerator ConstructTexture(GLTF.Schema.GLTFTexture texture, int textureIndex,
